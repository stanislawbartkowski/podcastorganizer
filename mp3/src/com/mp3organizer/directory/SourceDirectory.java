...
public class SourceDirectory {
     ...
     * Constructor
     * @param log Logger
     * @param p Converter (not used now)
     * @param iTest Testing interrupt (null if not active)
     */
    public SourceDirectory(final Logger log, final ConvertMp3File p, final ITestInterrup iTest) {
     ...
     */
    public final void createList(final String sourcedir, final DestDirectory destdir)
            throws Mp3Exception {
        ...
        for (FileDescr fd : dirDir.getFileList()) {
            ...
            for (FileDescr fmp3 : souDir.getFileList()) {
                ...
                if (destdir.isFile(fmp3.destbaseFileName)) {
                    log.info(fmp3.destbaseFileName + " already at destination, skip this file");
                    continue;
                } else {
...
//                break;
     ...
     * @param destPath Dest directory
     * @param iTest Interrupter (null if not disabled).
     * @throws Mp3Exception If any error occured
     */
    public final void copyFiles(final DestDirectory destdir, final String destPath,
